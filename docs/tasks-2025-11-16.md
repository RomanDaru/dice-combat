...
- **Execution log**:
  - `[2025-11-16 16:01:30] ✅ Refactored `releasePendingDefenseBuffs` to accept override buffers/skip-dispatch, added `triggerDefenseBuffsBatch`, and updated `useDefenseResolution` + its tests to fire both owners in a single pass.`
...
- **Execution log**:
  - `[2025-11-16 16:01:30] ✅ Added `mergeSnapshotWithPlayerSnapshots`, updated `startTurn` to re-read player snapshots after prep, and extended `turnStartLifecycle` tests so `resolveTurnStart` immediately consumes newly released DOT stacks.`
