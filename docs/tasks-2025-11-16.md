...
- **Execution log**:
  ...
  - `[2025-11-16 19:40:00] 🔁 Adjusted release timing so `postDefenseRoll` buffs now fire when defense resolution actually commits (on confirm/reaction success) instead of immediately after the roll. Player + AI controllers defer the trigger to `runDefenseResolution` / reaction handlers to keep confirmations authoritative.`
